...
.block-container { padding: 2.15rem 0.8rem 0.9rem !important; max-width: 1180px; }
div[data-testid="stVerticalBlock"] { gap: 0.12rem; }
[data-testid="stMarkdownContainer"], [data-testid="stMarkdownContainer"] * { text-align: right !important; }
h1, h2, h3, h4, h5, h6, p, label, .stCaption, .stMetric, .stSelectbox label, .stMultiSelect label, [data-testid='stWidgetLabel'], [data-testid='stWidgetLabel'] * { text-align: right !important; justify-content: flex-end !important; }
div[data-baseweb="select"] { min-height: 40px; }
div[data-baseweb="select"] > div { min-height: 40px !important; }
.stSelectbox, .stMultiSelect { margin-bottom: 0.15rem !important; }
[data-testid="stHorizontalBlock"] { gap: 0.32rem !important; }

/* ── Premium Header ── */
.app-header {
  background: linear-gradient(135deg, #0f1e3d 0%, #1a3560 55%, #1e4080 100%);
  border-radius: 16px; padding: 34px 28px 20px;
  margin-top: 18px; margin-bottom: 0; position: relative; overflow: hidden;
}
.app-header-top { display:flex; justify-content:space-between; align-items:flex-start; gap:18px; direction:ltr; }
.app-brand { text-align:left !important; }
.app-brand * { text-align:left !important; }
.app-header::before {
...
.app-logo { display: flex; align-items: center; gap: 12px; margin-bottom: 4px; justify-content:flex-start; }
.app-logo-icon {
...
.app-title { font-size: 38px; font-weight: 900; color: #f0f6ff; margin: 0; letter-spacing: -0.4px; line-height: 1.05; }
.app-sub   { font-size: 14px; color: #b8cee9; margin: 6px 0 0; line-height: 1.35; }
.app-header-stats { display: flex; gap: 14px; margin-top: 0; flex-wrap: wrap; justify-content:flex-end; }
.header-stat {
...
.nav-bar { 
  ...
  padding: 6px 10px 4px; border: 1px solid #e2e8f0; border-top: none;
  margin-bottom: 8px;
}
div[role="radiogroup"] {
...
div[role="radiogroup"] label:has(input:checked), div[role="radiogroup"] label:has(input:checked) *, div[role="radiogroup"] label[data-selected='true'], div[role="radiogroup"] label[data-selected='true'] * { color:#ffffff !important; fill:#ffffff !important; }
...
.profile-title { font-size: 16px; font-weight: 800; color: #1e3a8a; margin-bottom: 4px; }
.profile-hint  { font-size: 12px; color: #64748b; margin-bottom: 6px; }
...
.cmp-table td {
  padding: 6px 9px; border-bottom: 1px solid #f1f5f9;
...
def _get_votes_worksheet():
    ...
    try:
        ...
        votes_sheet_id = st.secrets.get("VOTES_GSHEET_ID", FUNDS_GSHEET_ID)
        sheet   = client.open_by_key(votes_sheet_id)
        try:
...
def _render_votes_stats(selected_alt: str = "", managers: str = "", tracks: str = ""):
    """Render voting statistics focused on the selected alternative."""
    df = _load_votes_cached()

    if df.empty:
        st.info("עדיין אין נתוני בחירה של משתמשים אחרים לחלופה זו.")
        return

    work = df.copy()
    if selected_alt and "alternative" in work.columns:
        exact = work[work["alternative"].astype(str).eq(str(selected_alt))]
        if not exact.empty:
            work = exact
    if managers and "managers" in work.columns:
        mgr_exact = work[work["managers"].astype(str).eq(str(managers))]
        if not mgr_exact.empty:
            work = mgr_exact
    if tracks and "tracks" in work.columns:
        track_exact = work[work["tracks"].astype(str).eq(str(tracks))]
        if not track_exact.empty:
            work = track_exact

    if work.empty:
        st.info("עדיין אין נתונים רלוונטיים לחלופה שנבחרה.")
        return

    total = len(work)
    unique_users = work["session_hash"].nunique() if "session_hash" in work.columns else total
    c1, c2, c3 = st.columns(3)
    c1.metric("בחירות רלוונטיות", f"{total:,}")
    c2.metric("משתמשים ייחודיים", f"{unique_users:,}")
    if "n_funds" in work.columns:
        avg_funds = pd.to_numeric(work["n_funds"], errors="coerce").dropna()
        c3.metric("מספר קופות ממוצע", f"{avg_funds.mean():.1f}" if not avg_funds.empty else "—")
    else:
        c3.metric("מספר קופות ממוצע", "—")

    if "mix_policy" in work.columns and work["mix_policy"].notna().any():
        counts = work["mix_policy"].astype(str).value_counts().head(3)
        st.markdown("**מדיניות מועדפת בקרב משתמשים דומים**")
        for name, cnt in counts.items():
            pct = 100.0 * cnt / max(total, 1)
            st.markdown(f"- {name}: **{cnt}** ({pct:.0f}%)")

    if "managers" in work.columns and work["managers"].notna().any():
        all_mgrs = []
        for cell in work["managers"].dropna():
            all_mgrs.extend([x.strip() for x in str(cell).split("|") if x.strip()])
        if all_mgrs:
            vc = pd.Series(all_mgrs).value_counts().head(4)
            st.markdown("**מנהלים בולטים בחלופות דומות**")
            for name, cnt in vc.items():
                pct = 100.0 * cnt / max(len(all_mgrs), 1)
                bar = max(8, int(180 * cnt / max(vc.max(), 1)))
                st.markdown(
                    f"<div class='stat-bar-wrap'><div style='min-width:110px;font-size:12px'>{_esc(name)}</div>"
                    f"<div class='stat-bar' style='width:{bar}px;background:#2563eb'></div>"
                    f"<div style='font-size:12px;color:#475569'>{cnt} ({pct:.0f}%)</div></div>",
                    unsafe_allow_html=True,
                )

    if "tracks" in work.columns and work["tracks"].notna().any():
        all_tracks = []
        for cell in work["tracks"].dropna():
            all_tracks.extend([x.strip() for x in str(cell).split("|") if x.strip()])
        if all_tracks:
            vc = pd.Series(all_tracks).value_counts().head(4)
            st.markdown("**מסלולים שנבחרו לרוב יחד עם החלופה הזו**")
            st.caption("הנתונים מבוססים על בחירות קודמות של משתמשים אחרים.")
            for name, cnt in vc.items():
                st.markdown(f"- {_esc(name)} — **{cnt}**")
...
def _gsheet_to_bytes(sheet_id: str) -> Tuple[bytes, str]:
    ...
    for url in urls:
        try:
            resp = _req.get(url, headers=headers, allow_redirects=True, timeout=12)
...
current_mgrs = st.session_state["selected_managers"]
placeholder_none = "__none__" in current_mgrs
current_mgrs = [m for m in current_mgrs if m in all_managers]
if not current_mgrs and not placeholder_none:
    current_mgrs = all_managers.copy()
st.session_state["selected_managers"] = current_mgrs if current_mgrs else (["__none__"] if placeholder_none else all_managers.copy())

n_active_managers = len([m for m in st.session_state["selected_managers"] if m != "__none__"])
...
if not real_selected:
    df_active = df_long.iloc[0:0].copy()
    st.warning("לא נבחרו מנהלים. בחר לפחות מנהל אחד כדי לראות קרנות וחלופות.")
elif len(real_selected) < n_total_managers:
...
if st.session_state["quick_profile_active"]:
    ...
    st.caption(f"🎯 מסנן פעיל: {pname} — מציג קרנות עם חשיפה ≥90%")
...
if active_tab == "⚙️ הגדרות יעד":
    st.markdown("""
    <h3 style="margin: 6px 0 10px 0; text-align: right; font-size: 22px; font-weight: 800; color: #1e3a8a;">בניית תמהיל מסלולים מותאם אישית</h3>
    """, unsafe_allow_html=True)
    ...
    st.divider()
    run = st.button("🔍 חשב חלופות מיטביות", type="primary", use_container_width=True, disabled=df_active.empty)
    if df_active.empty:
        st.caption("בחר לפחות מנהל אחד כדי לאפשר חישוב חלופות.")
        ...
        with st.spinner("מחשב... ⚡ (חיפוש מואץ עם NumPy)"):
            try:
                ...
                if sols.empty:
                    ...
                    st.session_state['_scroll_results_now'] = True
                    st.toast('החלופות חושבו ומוצגות מיד מתחת לכפתור', icon='✅')
                    st.markdown("""
<script>
setTimeout(function(){
  ...
  if(el){
    ...
    if res is not None and active_tab == "⚙️ הגדרות יעד":
        ...
        if recs:
            ...
            if st.session_state.pop('_scroll_results_now', False):
                st.markdown("""<script>
                setTimeout(function(){
                  var el = window.parent.document.getElementById('results-anchor');
                    ...
                    if d is None:
                        ...
                        if col_key == "שארפ משוקלל" and is_alt:
                            if d.get("sharpe_incomplete") or (isinstance(val, float) and math.isnan(val)):
                                cells += "<td style='font-size:10px;color:#92400e'>לא כל המסלולים בחלופה<br>פרסמו מדד שארפ</td>"
                                continue
                        dstyle = ""
...
<div style='overflow-x:auto;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;margin-bottom:6px'>
  <table class='res-tbl'>
    ...
    <tbody>{body_html}</tbody>
            ...
            st.caption("בחר חלופה בשורה שמתחת לטבלה כדי לראות מייד פרטים וסטטיסטיקה של משתמשים דומים")
                    ...
                    if sel_row:
                        ...
                        with dc3:
                            ...
                            if sel_row.get("sharpe_incomplete") or (isinstance(sv, float) and math.isnan(sv)):
                                st.metric("שארפ", "—")
                                st.caption("⚠️ לא כל המסלולים בחלופה פרסמו מדד שארפ")
                            else:
                        ...
                        with dc4:
                            ...
                            if trk_list:
                                st.markdown("**מסלולים:**")
                                for idx, t in enumerate(trk_list):
                                    mgr = mgr_list[idx] if idx < len(mgr_list) else ""
                                    lbl = f"{mgr} — {t}" if mgr else t
                                    st.markdown(f"• {lbl}")

                if voting_configured:
                    st.divider()
                    st.markdown("**📊 איך משתמשים אחרים בחרו חלופות דומות**")
                    _render_votes_stats(selected_alt=selected_alt, managers=str(sel_row.get("מנהלים","")) if sel_row else "", tracks=str(sel_row.get("מסלולים","")) if sel_row else "")
                else:
...
if active_tab == "⚖️ השוואת מסלולים":
    ...
    if qp:
        ...
        with col_banner:
            st.caption(f"🎯 מסלול מהיר פעיל: {qp} — {' | '.join(filter_parts)}")
        with col_clear:
    ...
    if not unique_rows:
        ...
        .cmp-table th {{
          background:#0f172a; color:#f8fafc; padding:6px 8px;
          font-weight:700; white-space:nowrap; text-align:right;
        }}
        .cmp-table td {{
        ...
        if not bar_df.empty:
            ...
            fig_bar.update_layout(
                height=220, margin=dict(t=12, b=50, l=8, r=8),
...
st.caption("© Profit Mix Optimizer v3.1 | ישראל = 100% − חו״ל | חיפוש מואץ עם NumPy vectorized")
